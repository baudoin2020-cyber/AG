...
def convert_and_run_and_save_image(code_java, nom_image):
    ...
    if not code_python.lstrip().startswith(
        "from ag2 import *"
    ):
        code_python = (
            "from ag2 import *\n\n"
    ...
    # Chaque appel doit commencer avec un dessin neuf.
    ...
    fonction_save = espace.get("save")

    if not callable(fonction_save):
        raise RuntimeError(
            "La fonction save() de ag2.py est introuvable."
        )

    fonction_save(chemin_image)
